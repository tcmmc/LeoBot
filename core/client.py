...
class RubikaClient:
    ...
    def register_update_handler(self, dispatch_callback):
        ...
        async def _on_update(rubpy_client, message):
            self._capture_loop()
            self.submit_sync_task(dispatch_callback, self, message)

        registered = False

        # تلاش برای ثبت بدون فیلتر (دریافت همه‌ی آپدیت‌ها شامل PV، گروه و دکمه‌ها)
        try:
        ...
        # اگر نسخه‌ی نصب‌شده حتماً فیلتر می‌خواهد، PV و گروه را جدا ثبت می‌کنیم
        if not registered:
            try:
                combined = self.filters.private | self.filters.group
                self._client.on_update(combined)(_on_update)
                registered = True
            except Exception:
                try:
                    self._client.on_update(self.filters.private)(_on_update)
                    self._client.on_update(self.filters.group)(_on_update)
                    registered = True
                except Exception as exc:
                    log.error(f"ثبت هندلر on_update ناموفق بود: {exc}")

        if not registered:
            raise RuntimeError(
                "امکان ثبت هندلر پیام روی BotClient وجود ندارد؛ نسخه‌ی rubpy را بررسی کنید."
            )
