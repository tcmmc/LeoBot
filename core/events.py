...
def _dispatch_message(client, message):
    ...
    log.debug(
        f"دیسپچ آپدیت -> chat_guid={chat_guid!r} user_guid={user_guid!r} "
        f"button_id={button_id!r} text={text[:40]!r}"
    )

    if not chat_guid or not user_guid:
        log.warning(
            "آپدیت نادیده گرفته شد چون chat_guid یا user_guid استخراج نشد. "
            "برای دیدن ساختار خام آپدیت، LOG_LEVEL=DEBUG را در .env/Variables تنظیم کنید."
        )
        return
...
def _handle_private_message(client, message, user_guid):
    ...
    is_first_contact = db.get_bot_user_row(user_guid) is None

    # بعضی نسخه‌های Bot API رسمی، رویداد «Start» را با متن خالی (نه واژه‌ی
    # «شروع») ارسال می‌کنند؛ برای همین اولین تماس هر کاربر هم Start حساب می‌شود.
    if normalized in ("شروع", "start", "Start", "/start") or (not normalized and is_first_contact):
